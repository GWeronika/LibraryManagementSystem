...
public class OrderDAO extends GenericDAO<Order> {
    ...
    public void addOrderToDB(Order order) {
        String query = "INSERT INTO orders (orders_date, status, reader_id, copy_id) VALUES (?, ?, ?, ?)";
    ...
    public void selectOrderFromDB() {
        String[] columns = {"orders_date", "orders.status", "book.title", "book.author"};
    ...
    public Order getOrderByID(int orderID) {
        ...
        for (Result result : resultList) {
            int resultOrderID = Integer.parseInt(result.getColumnValues().get("orders_id"));

            if (resultOrderID == orderID) {
                LocalDate orderDate = LocalDate.parse(result.getColumnValues().get("orders_date"));
                ...
                return new Order(resultOrderID, copyDAO.getCopyByID(copyID), orderDate, status, readerDAO.getReaderByID(readerID));
            }
        }
        return null;
    }
    ...
    private List<Result> extractFromDB(int id) {
        String[] columns = {"orders_id", "orders_date", "orders.status", "reader_id", "copy_id"};
        String condition = "orders_id = ?";
